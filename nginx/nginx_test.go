		...
		context("detects an NGINX app", func() {
			...
			context("app uses no configuration NGINX", func() {
				it.Before(func() {
					...
					source, err = occam.Source("../nginx/no-config-file-sample/app")
					Expect(err).NotTo(HaveOccurred())
				})
				...
				context("when the user sets https forced redirect", func() {
					it("serves a static site that always redirects to https", func() {
						...
						Expect(err).To(MatchError(`Get "https://localhost/": dial tcp 127.0.0.1:443: connect: connection refused`))
					})
				})
				...
				context("when the user provides a basic auth binding", func() {
					...
					it.Before(func() {
						var err error
						binding, err = filepath.Abs("../nginx/no-config-file-sample/binding")
						Expect(err).NotTo(HaveOccurred())
					})
